# Quickstart Guide: Phase II Persistent CLI Task Manager

## Overview
This guide provides instructions for setting up and running the persistent CLI task manager with PostgreSQL storage.
...
3. Set up the database:
   ...
   - Update the database connection settings in environment variables:
     ```
     DATABASE_URL=postgresql://username:password@host:port/database_name
     ```

4. Navigate to the CLI app directory:
   ```bash
   cd apps/cli
   ```
...
If migrating from Phase I (in-memory only), use the migration tool:
```bash
python migrations/migration_tool.py --source memory --destination postgres
```